...
**HTML mockup as input — set expectations honestly.** An HTML mockup is **read as reference, not ported pixel-for-pixel.** You review the narrative in it, distill it into the brief, and slide-builder then **regenerates the slides from scratch on the client template's real layouts** (its sketch path authors fresh HTML per slide from the brief and converts it to native PowerPoint). The mockup's exact visual design does not carry through — its content and structure do. Tell the user this before building so they don't expect a 1:1 copy of their mockup.
