...
| Code | Meaning | Fix |
|---|---|---|
| 5  | Refused: no valid `--review-token` (or a stale/mismatched one, or `--out` not the canonical dir). The final deck is HUMAN-gated: the token is minted by `build_review.py` and shown only inside `REVIEW.html`. | Build `REVIEW.html`, have the user pick, copy the token from its 'Build my deck' command, then compile with `--review-token <token>`. A (re)build invalidates the old token, so re-review first. |
