# Slide 1 build prompt — `OTC body light - inherited title placeholder`

You are building **slide 1 of 3** of a deck using `slide-builder` (v2). Produce **three structurally distinct options** as standalone runnable Python scripts: `option_A.py`, `option_B.py`, `option_C.py`. Each script builds **this single slide** against the client template and is executed by `finalize_deck.py` at finalize time.

You are one of 3 parallel agents dispatched from the same parent session. The other agents are building the other slides at the same time. You see only your slide's brief content. You do not see the other slides' content directly, but you do see the patterns picked for the previous two slides — those constrain you via the adjacency rule (Hardline #3).

---

## 1. Brief content (only this slide)

**Slide title:** OTC body light - inherited title placeholder

**Governing thought (the claim):**
v0.3 keeps the OTC body layout chrome.

**So-what (the takeaway):**
Title lands in the inherited title placeholder; layout chrome inherits.

**Editorial emphasis:**
(none specified)

**Evidence / content:**
Rendered slide carries Rectangle 4/5/6 (OTC chrome) plus a populated title placeholder.
...
1. **Layout catalog** (the 14 patterns + 1 fallback):
   ```
   C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\layouts.md
   ```
...
2. **Anti-pattern library** (the don't-library, scanned once preventively):
   ```
   C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\anti-patterns.md
   ```
...
2. **Take the highest-scoring pattern.** If two or more tie, use the rotation seed.
   ...
   - Your pattern-pick seed: `704af4d92fcf806898401dc7114c43b9`
...
3. **Adjacency context (Hardline #3 — soft-enforced here, hard-enforced at the gate-preview + review steps via `build_gate_preview.py` + `build_review.py`).** Likely prior patterns from the prep-time hint pass:
   ```
   (this is the first slide — no prior context)
   ```
...
4. **Check the fallback trigger.** If the slide concept implies a curved-container diagram, route to fallback. Two cases:
   ...
   **(a) v0-supported via Mermaid: hub-and-spoke, Porter's Five Forces, ecosystem map, free-form network.** For each of the three options:
   ...
   - Write a sibling `option_X.mmd` file in the same `C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01` directory. This is your Mermaid spec — finalize_deck.py renders it to PNG (at body-zone size 1240×540) and embeds the PNG in the slide.
   - Read `C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\fallback.md` for the full fallback contract, the supported Mermaid diagram types per failure case, and the brand theme override mechanism.
   - Read the worked examples at `C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\fallback-examples` (`hub-spoke.mmd`, `porters-five-forces.mmd`) for the structural pattern matching your brief. Adapt the example to your brief's content; preserve the `classDef` block (Mermaid uses these for brand color emphasis).
...
PATTERN PICK — Slide 1
  ...
  Seed used?    : <yes/no — yes if you tiebroke with 704af4d92fcf806898401dc7114c43b9>
...
- Use the per-option variant seeds to vary your starting variant choice — each of the three options has its own seed so the three end up on different variants. The seeds also prevent 5 parallel agents on the same brief from picking the same variant set.
  - Option A variant seed: `1c5595c9722e1249a9ce0f2056bb723f`
  - Option B variant seed: `f7737e075e38735f8672b4b3c9832285`
  - Option C variant seed: `14350362c3be34972b22b0b1c0322ace`
...
These are non-negotiable. Every option script must satisfy all five. The full text lives at `C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\SKILL.md`; inline summary below.
...
Write three files to `C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01`:

```
C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01\option_A.py
C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01\option_B.py
C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01\option_C.py
```
...
1. Imports from `slide-builder\twins\helpers.py` — the shared chrome helpers (title block, footer, brand colors, primitives). Add the absolute path to `sys.path` at the top of each script:
   ```python
   import sys
   sys.path.insert(0, r"C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder")
   ...
   ```
2. Does **NOT** open the client template directly. Build slide content fresh on a 1280×720 canvas using `new_slide()` from `twins.helpers` — this returns `(prs, slide)` with no template. `finalize_deck.py` handles the graft of your slide onto `C:\Users\m.a.peralta\OneDrive - Accenture\Library\FedEx\OTC\OTC Opportunity.pptx` after your script saves. The client template path is provided in § 11 as context only; your script does not need to load it.
...
```python
# Slide 1 option <A|B|C> — pattern: <picked pattern name>
...
```python
# SKELETON_REJECTED: <one-line reason — e.g., brief enumerates 2 items, pattern needs 4 cells>
# Slide 1 option <A|B|C> — pattern attempted: <pattern name>
import sys
sys.exit(0)
```
...
And in the same `C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01`:
...
```
Layout catalog          : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\layouts.md
Anti-pattern library    : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\anti-patterns.md
Fallback contract       : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\fallback.md        (read only if fallback trigger fires)
Fallback examples       : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\reference\fallback-examples   (worked Mermaid specs)
SKILL.md                : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\SKILL.md
Helpers module          : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder
Client template         : C:\Users\m.a.peralta\OneDrive - Accenture\Library\FedEx\OTC\OTC Opportunity.pptx
Output directory        : C:\Users\m.a.peralta\.claude\skills\.claude\worktrees\agent-abaae9e70ed8f6544\slide-builder\_decisions\v0.3-smoke\build\slide_01
```
