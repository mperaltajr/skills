...
def write_brand_css(path: Path, *,
    ...
    content = (
        ...
        f" */\n"
        f":root {{\n"
        f"  /* Brand colors (from brand.yml) */\n"
        f"  --brand-primary:        #{p};\n"
        f"  --brand-accent:         #{a};\n"
        f"  --brand-cover-bg:       #{cbg};\n"
        f"  --brand-dark-bg:        #{dbg};\n"
        f"\n"
        f"  /* Derived tones (mechanical mixes; Spec 2 §7) */\n"
        ...
        f"  --brand-border-light:   #D3D1C7;\n"
        f"  --brand-divider:        #E1E1E6;\n"
        f"\n"
        f"  /* Fonts */\n"
        f"  --font-heading: \"{fh}\", \"Segoe UI\", -apple-system, sans-serif;\n"
...
def _write_outputs(tpl: Path, sha: str, sha8: str,
    ...
    # systems deliberately use low-contrast pairings. Surface as a
    # registration-time stderr notice so the operator sees it before
    # building decks.
    from twins.client_theme import wcag_contrast
    ratio = wcag_contrast(primary_hex, accent_hex)
    if ratio < 4.5:
        sys.stderr.write(
            f"  NOTICE (WCAG AA): brand primary #{primary_hex.upper()} on "
            f"accent #{accent_hex.upper()} has contrast {ratio:.2f}:1 "
            f"(below 4.5:1). Slides that combine these two colors for text "
            f"may be hard to read on projection. This is informational; "
            f"the build will proceed normally.\n"
        )
...
from _chrome_schema import (
    ...
    emu_to_px as _emu_to_px_canonical,
    px_to_emu as _px_to_emu_canonical,
)
...
def _emu_to_px(emu: int) -> int:
    ...
    Thin None-safe wrapper around `_chrome_schema.emu_to_px` (the canonical
    helper). Kept under the original `_emu_to_px` name so the many internal
    call sites in this module continue to work unchanged.
    """
    if emu is None:
        return 0
    return _emu_to_px_canonical(emu)
