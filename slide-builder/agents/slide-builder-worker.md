...
4. **Produce the option(s) the prompt asks for** (§5 of `_prompt.md` — one by default, more only if the count says so) within the chosen pattern. At least one option must explicitly honor the directive verb. Use the per-option variant seed(s) the prompt lists to vary your starting variant choice.
...
5. **Write `_context_ack.txt`** in the same `slide_NN/` directory as `_context.md` and `_prompt.md`. This is a single-line file (max ~200 chars) that cites ONE specific constraint from `_context.md` that informed your pattern pick or variant choice. Examples of valid lines:
   ...
   - `>2-line title rule — title is 88 chars / 2 visual lines, kept subtitle in the option(s).`
...
6. **Write the option script(s) the prompt specifies** (the exact file list in the prompt's §8 Output contract — one by default) to the output directory. The output format depends on the `PATTERN` field in `_prompt.md` (or the dispatch message). Two branches:

   ### Direct path (default — python-pptx direct)

   When `PATTERN: direct` or the field is absent, write python-pptx script(s) — the exact file(s) the prompt lists, e.g.:

   ```
   <out_dir>/slide_NN/option_A.py
   ```
   (more — `option_B.py`, `option_C.py` — only when the prompt's count asks)
   ...
   ```
   (more — `option_B.html`, `option_C.html` — only when the prompt's count asks)
...
- Absolute paths of the option file(s) written
...
- **Editorial intent ambiguous** (brief doesn't map to any of the 7 directive verbs): write the option file(s) (per the prompt's count) with `# SKELETON_REJECTED: ambiguous editorial intent — brief does not map to {recommend, warn, diagnose, show urgency, show progress, compare neutrally, summarize}` on line 1.
- **Pattern/brief disagreement** (Hardline #5): write the option file(s) with `# SKELETON_REJECTED: <specific disagreement reason>` on line 1.
