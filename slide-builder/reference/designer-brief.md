...
- **Visual rule violations are taught by anti-exemplars, not by prose rules.** For any visual / aesthetic question (whitespace, balance, contrast, accent placement, reading order, table headers, title-vs-accent proportions, vertical-rule gutters, etc.) — look at the `exemplars/dont/` library for concrete failure references. Each `dont/<slug>/` includes the bad slide as PNG plus a `WHY.md` explaining which rule it violates and what to do instead. The `dont/` library is the canonical teaching mechanism; this rulebook keeps only mechanical invariants (canvas size, font floor, brand palette, etc.).
...
- **Cover / Divider** — full-bleed dark with hero title. Tagline below in `BRAND_ACCENT_SOFT`. Single accent rule. **Hero title size cap: 36-48px for multi-word titles.** 60-96px is reserved for single-numeral hero slides (KPI tile), not cover titles. A multi-word cover title at 72px+ renders as chunky/Impact-style regardless of font family — Graphik, FedEx Sans, Inter all suffer at that scale. Stay editorial. **Cover slides bypass `add_title_block` and build their own title with `add_text` at the cover-appropriate size (see cover exemplars for the convention).**
...
- **Data Tables** — when the slide IS the table (risk register, scoring matrix, decision matrix, status-by-workstream, KPI dashboard, vendor comparison). Use `add_table(slide, ...)` with mandatory column headers. Header row in `BRAND_PRIMARY` band with `WHITE` text, banded rows for legibility, right-aligned numeric columns, ONE column or row highlighted as the accent moment. Cell-treatment patterns (harvey balls, RYG status pills, up/down arrows, mini-bars, sparklines) overlay shapes on top of the table after the helper draws it. Parallel rows of numeric values that don't strictly use `add_table` (e.g., McKinsey-style structured columns) still need labeled column headers — the rule applies to any tabular content.
