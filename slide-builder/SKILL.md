...
10. **QC — mandatory, not optional.** Invoke **slide-qc**, passing it the compiled `final_deck.pptx` path and the session folder so it doesn't have to re-discover them. This is the definition of done: do not tell the user the deck is finished or "QC'd" until slide-qc has run and produced its report. A PDF you rendered and eyeballed is not QC — the agent that built the deck cannot grade its own output.
