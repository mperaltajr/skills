"""
Builder for pattern 88: Status report — executive summary (quadrant with header-bar).

Source HTML: _pattern-library/88_status-report-exec-summary.html

Variant title: brand-primary band header bar replaces standard title block —
no separate brand-rule used; the rule sits as a 4px accent across the slide.
"""
...
RAG_GREEN_SOFT = RGBColor(0xD6, 0xF5, 0xE3)
RAG_AMBER = RGBColor(0xF3, 0x9C, 0x12)
RAG_RED = RGBColor(0xDC, 0x26, 0x26)
RAG_RED_BG = RGBColor(0xFD, 0xEC, 0xEC)


def build():
    prs, slide = new_slide()

    # Variant chrome — sits above the brand bar

    # Header bar — brand-primary band, white title inside
    add_rect(slide, "header-bar", 0, 36, 1280, 68, BRAND_PRIMARY)
    add_text(slide, "title",
             "Pilot status · Week 3 · Green overall, three decisions needed.",
             x_px=40, y_px=50, w_px=1200, h_px=46,
             font_size_px=22, color=WHITE, bold=True)
    # Brand-rule under the bar (full width)
    ...
    # Quadrant grid coords from HTML
    quads = {
        "tl": (40, 148, 590, 220, "Overall Status", "Week 3 of 4", BRAND_PRIMARY, BRAND_ACCENT_SOFT),
        "tr": (650, 148, 590, 220, "Key Wins This Period", "4 wins", BRAND_PRIMARY, BRAND_ACCENT_SOFT),
        "bl": (40, 384, 590, 232, "Key Risks", "3 open", RAG_RED, RGBColor(0xFF, 0xE8, 0xE8)),
        "br": (650, 384, 590, 232, "Decisions Needed", "3 asks", BRAND_ACCENT, RGBColor(0xF4, 0xE3, 0xFF)),
    }

    for pos, (x, y, w, h, name, meta, head_color, meta_color) in quads.items():
        # Body backgrounds
        body_color = RAG_RED_BG if pos == "bl" else CARD_BG
        ...
        # Icon placeholder square top-left of head
    ...
    for i, (ask, owner) in enumerate(asks):
        ...
        ay = ask_top + i * stride
        # Decision card
        dcard = add_rect(slide, f"sub-ask-{n}", br_x + 14, ay, br_w - 28, card_h, WHITE)
    ...
    # Convergence band — pink-accent on brand-primary
    conv_y = 720 - 56 - 40
    conv_h = 40
    add_rect(slide, "convergence-bg", 40, conv_y, 1280 - 80, conv_h, BRAND_PRIMARY)
    ...
    add_footer(slide, page_num=88)
    return prs


if __name__ == "__main__":
    out = Path(__file__).resolve().parents[2] / "_renders" / "twins" / "88_status-report-exec-summary.pptx"
