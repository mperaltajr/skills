"""
Builder for pattern 37: Comparison tier cards (3 options, middle is recommended).

Three identical-height cards. Middle card (card-2) has brand-primary fill,
accent border, and a RECOMMENDED badge.

Pattern-local IDs: card-N-tagline, card-N-feat-M, card-N-feat-M-icon,
card-N-feat-M-text, card-N-stat-label, card-N-stat-value, card-2-badge.

Source HTML: _pattern-library/37_comparison-tier-cards.html
"""
...
CHECK_MEH = RGBColor(0xCA, 0x8A, 0x04)
...
def build():
    ...
    add_title_block(
        slide,
        title="Three rollout options — pick one to learn from, not three to maintain.",
        subtitle="Two of these defer the decision; one gives us the evidence to make it. The middle path is sized to learn fast and reverse cleanly.",
        title_h=64,
        subtitle_h=22,
    )

    # Cards row: top=180, height=420, gap=20, padding 64
    cards_top = 180
    cards_left = 64
    cards_right = 1280 - 64
    ...
    # 5 features per card: (icon_state, text)
    features = [
    ...
    for i in range(3):
        ...
        # Card background
        if is_rec:
            card = add_rect(slide, f"card-{n}", cx, cards_top, card_w, card_h, BRAND_PRIMARY)
            card.line.color.rgb = BRAND_ACCENT
            card.line.width = 25400
        else:
            card = add_rect(slide, f"card-{n}", cx, cards_top, card_w, card_h, CARD_BG)
        ...
        eyebrow_color = BRAND_ACCENT_SOFT if is_rec else BRAND_ACCENT
        name_color = WHITE if is_rec else BRAND_PRIMARY
        tagline_color = BRAND_ACCENT_SOFT if is_rec else TEXT_MID
        ...
        # Features (5 rows × 36px)
        feat_top = cards_top + 122
        feat_row_h = 36
        for fi, (state, ftext) in enumerate(features[i]):
            fn = fi + 1
            fy = feat_top + fi * feat_row_h
            # Divider rule (top of each feature except first)
            if fi > 0:
                rule_color = RGBColor(0xFF, 0xFF, 0xFF) if is_rec else CARD_BORDER
            ...
            add_text(
                ...
                font_size_px=12, color=WHITE if is_rec else TEXT_DARK,
            )

        # Footer stat
        stat_y = cards_top + card_h - 56
        stat_rule_color = BRAND_ACCENT if is_rec else CARD_BORDER
        add_rect(slide, f"card-{n}-stat-rule", cx + 22, stat_y, card_w - 44, 2, stat_rule_color)
        stat_label_color = BRAND_ACCENT_SOFT if is_rec else TEXT_FAINT
        add_text(
        ...
        add_text(
            ...
            font_size_px=13, color=WHITE if is_rec else BRAND_PRIMARY, bold=True,
        )
    ...
    add_footer(slide, page_num=37)
    return prs


if __name__ == "__main__":
    out = Path(__file__).resolve().parents[2] / "_renders" / "twins" / "37_comparison-tier-cards.pptx"
