"""
Dark variant of pattern 23: Account scorecard — RAG grid.

Source HTML: _pattern-library/23_account-scorecard-rag-grid-dark.html
"""
...
AMBER_BG_DARK = RGBColor(0x4A, 0x35, 0x1F)  # warm tinted dark for amber body
...
def build():
    ...
    # Brand header bar (sits below chrome) — slightly lighter on dark bg
    add_rect(slide, "title-band", x_px=0, y_px=44, w_px=1280, h_px=68, fill_color=CARD_BG_DARK)
    add_text(
    ...
    add_rect(slide, "brand-rule", x_px=0, y_px=112, w_px=1280, h_px=4, fill_color=BRAND_ACCENT_SOFT)
    ...
    for i, (prefix, name, rag, amber_body, sub_label, lines) in enumerate(card_data):
        ...
        # Card body (dark)
        card = add_rect(slide, f"{prefix}-bg", cx, cy, card_w, ch, CARD_BG_DARK)
        card.line.color.rgb = CARD_BORDER_DARK
        card.line.width = 9525

        # Head (slightly different shade to read above body)
        head_h = 42
        add_rect(slide, f"{prefix}-head", cx, cy, card_w, head_h, RGBColor(0x4A, 0x29, 0x6D))
        # RAG dot
        ...
        # Body
        body_bg = AMBER_BG_DARK if amber_body else CARD_BG_DARK
...
if __name__ == "__main__":
    out = Path(__file__).resolve().parents[2] / "_renders" / "twins" / "23d_account-scorecard-rag-grid.pptx"
