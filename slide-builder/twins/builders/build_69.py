"""
Builder for pattern 69: Tornado sensitivity chart with annotation panel.

Tornado is bar-chart-like (6 rows with positive/negative bars around centerline).
We render each bar natively (decompose) since bar-N-* IDs are addressable.
The grid/axis is the chart-canvas placeholder.

Source HTML: _pattern-library/69_tornado-sensitivity.html
"""
...
def build():
    ...
    add_rect(slide, "brand-rule", 56, 142, 56, 3, BRAND_ACCENT)

    # Layout: chart panel (left) + annot panel (right)
    g_top = 180
    g_left = 56
    g_right = 1280 - 56
    g_bottom = 720 - 80 - 56  # leave room for convergence + footer
    g_w = g_right - g_left
    g_h = g_bottom - g_top
    annot_w = 280
    gap = 18
    ...
    cp = add_rect(slide, "chart-panel-bg", g_left, g_top, chart_w, g_h, CARD_BG)
    cp.line.color.rgb = CARD_BORDER
    cp.line.width = 9525
    ...
    # "Baseline: 14 days" — moved out of the cramped top-right into open
    # space below the bars, just right of the leftmost tick (-6d). This
    # lines it up with the x-axis annotation and stops crowding the chart
    # title row.

    # Legend — top-right inside chart panel, aligned with chart title row
    # Item 2 right-aligned to chart panel right edge (g_left + chart_w - 12)
    leg_y = g_top + 14
    leg2_label_w = 130
    leg2_sw = 10
    leg2_gap = 6
    ...
    add_rect(slide, "legend-1-swatch", leg1_x, leg_y + 3, leg1_sw, leg1_sw, BRAND_PRIMARY_MID)
    add_text(
    ...
    # Chart-canvas placeholder for tick grid + axis labels (we render bars natively)
    chart_canvas_y = g_top + 58
    chart_canvas_h = g_h - 68
    canvas_x = g_left + 12
    canvas_w = chart_w - 24

    # Plot-area mapping: 7 ticks (-6,-4,-2,0,+2,+4,+6), centerline at center
    # Each unit = canvas_w / 12
    # 160px label gutter on left so long row labels flow horizontally (was 60)
    ...
    add_rect(slide, "chart-baseline-axis",
             centerline_x, chart_canvas_y, 2, chart_canvas_h - 30, BRAND_PRIMARY_MID)
    ...
    # Baseline annotation — placed in the open whitespace just to the
    # right of the -6d tick mark (left of the centerline), level with the
    # axis tick row so it reads as part of the axis caption rather than
    # crowding the chart-title row.
    add_text(
    ...
    for ri, (label, neg, pos) in enumerate(rows):
        ...
        # Label (left of centerline)
        add_text(
            slide, f"bar-{n}-label", label,
            x_px=canvas_x + 6, y_px=ry + (row_h - 16) // 2, w_px=plot_left - canvas_x - 12,
            h_px=18, font_size_px=10, color=TEXT_DARK, bold=True, align="right",
        )

        # Negative bar (from centerline leftward)
        if neg < 0:
            nw = int(abs(neg) * unit_px)
            add_rect(slide, f"bar-{n}-neg-bar",
                     centerline_x - nw, bar_y, nw, bar_h, BRAND_PRIMARY_MID)
            add_text(
                slide, f"bar-{n}-neg-val", f"{neg}d",
                x_px=centerline_x - nw - 36, y_px=bar_y + 2, w_px=32, h_px=18,
                font_size_px=10, color=BRAND_PRIMARY_MID, bold=True, align="right",
            )
        # Positive bar (from centerline rightward)
        if pos > 0:
            pw = int(pos * unit_px)
    ...
    add_rect(slide, "annot-bg", ap_x, g_top, annot_w, g_h, BRAND_PRIMARY)
    add_text(
    ...
    add_footer(slide, page_num=69)
    return prs


if __name__ == "__main__":
    out = Path(__file__).resolve().parents[2] / "_renders" / "twins" / "69_tornado-sensitivity.pptx"
